...
def main(args):
    ...
    print("---text_encoder_cls_one loaded---")
    text_encoder_cls_two = import_model_class_from_model_name_or_path(
        args.pretrained_model_name_or_path, args.revision, subfolder="text_encoder_2"
    )
    print("---text_encoder_cls_two loaded---")
    ...
    # def load_model_with_mmap():
    #     model_path = os.path.join(args.pretrained_model_name_or_path, "transformer")
        
    #     # 加载配置
    #     config = FluxTransformer2DModel.load_config(model_path)
    #     model = FluxTransformer2DModel(config)
        
    #     # 使用mmap加载权重
    #     for file in os.listdir(model_path):
    #         if file.endswith('.safetensors'):
    #             weights = load_file(os.path.join(model_path, file), device='gpu')
    #             model.load_state_dict(weights, strict=False)
    #             print(f"Loaded {file}")
                
    #     return model.to(accelerator.device)

    # transformer = load_model_with_mmap()
    
    # Different Methods tried:
    # 1. direct loading
    # 2. + low_cpu_mem_usage=True
    # 3. + init_empty_weights() & device map
    # 4. load with mmap
    ...
    transformer = FluxTransformer2DModel.from_pretrained(
        ...
        revision=args.revision,
        variant=args.variant,
        torch_dtype=weight_dtype,  # 确保混合精度
        device_map="auto",         # 自动分配至GPU
        low_cpu_mem_usage=True,     # 减少CPU内存峰值
        offload_folder=None,         # 禁用 offload 目录
        sequential_cpu_offload=False # 明确关闭顺序卸载
    )
    ...
    def move_to_cuda(batch):
        """Move batch items to CUDA immediately after loading."""
        for key in batch:
    ...
    train_dataloader = torch.utils.data.DataLoader(
        ...
        shuffle=True,
        collate_fn=lambda b: move_to_cuda(collate_fn(b)),
        num_workers=args.dataloader_num_workers,
        pin_memory=False,  # Set to False since we're moving to CUDA in collate
    )
    ...
    logger.info(f"  width of generation: {args.width}, height of generation: {args.height}")
